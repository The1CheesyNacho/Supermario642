...
try:
    files = os.listdir("assets/wario_anims")
    files.sort()

    for filename in files:
        if filename.endswith(".inc.c"):
            lines = []
            with open("assets/wario_anims/" + filename) as f:
    ...
    for item in items:
        ...
        if type == "header":
            ...
            offset_to_struct = "offsetof(struct WarioAnimsObj, " + name + ")"
            offset_to_end = "offsetof(struct WarioAnimsObj, " + values + ") + sizeof(gWarioAnims." + values + ")"
    ...
    for item in items:
        ...
        if type == "header":
            ...
            structobj.append("{" + ", ".join([
                ...
                str(v5),
                str(indices_len),
                "(const s16 *)(offsetof(struct WarioAnimsObj, " + values + ") - " + offset_to_struct + ")",
                "(const u16 *)(offsetof(struct WarioAnimsObj, " + indices + ") - " + offset_to_struct + ")",
    ...
    print("const struct WarioAnimsObj {")
    for s in structdef:
        print(s)
    print("} gWarioAnims = {")
    for s in structobj:
    ...
    note = "NOTE! The wario animation C files are not processed by a normal C compiler, but by the script in tools/wario_anims_converter.py. The format is much more strict than normal C, so please follow the syntax of existing files.\n"
    if e is SyntaxError:
        e.msg = note + e.msg
    else:
