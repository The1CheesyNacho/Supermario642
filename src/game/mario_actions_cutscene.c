...
s32 act_standing_death(struct MarioState *m) {
    ...
    play_sound_if_no_flag(m, SOUND_CHARACTER_DYING, MARIO_ACTION_SOUND_PLAYED);
...
s32 act_quicksand_death(struct MarioState *m) {
    ...
    if (m->actionState == ACT_STATE_QUICKSAND_DEATH_SINK) {
        if (m->quicksandDepth >= 100.0f) {
            play_sound_if_no_flag(m, SOUND_CHARACTER_WAAAOOOW, MARIO_ACTION_SOUND_PLAYED);
        }
...
s32 act_shocked(struct MarioState *m) {
    ...
    if (m->actionArg == 0) {
        ...
        if (m->actionTimer >= 6) {
            m->invincTimer = 30;
            set_mario_action(m, gMarioStates[0].health < 0x0100 ? ACT_ELECTROCUTION : ACT_IDLE, 0);
        }
        stop_and_set_height_to_floor(m);
    }
        ...
        case ACT_STATE_SQUISHED_SCALING:
            ...
            if (spaceUnderCeil >= 10.1f) {
                ...
                if (!(m->flags & MARIO_METAL_CAP) && m->invincTimer == 0) {
                    ...
                    play_sound_if_no_flag(m, SOUND_CHARACTER_ATTACKED, MARIO_MARIO_SOUND_PLAYED);
                }
...
static void intro_cutscene_jump_out_of_pipe(struct MarioState *m) {
    ...
    if (m->actionTimer++ >= 118) {
        ...
        if (perform_air_step(m, AIR_STEP_CHECK_NONE) == AIR_STEP_LANDED) {
            ...
            play_mario_landing_sound(m, SOUND_ACTION_TERRAIN_LANDING);
            play_sound(SOUND_CHARACTER_HAHA, m->marioObj->header.gfx.cameraToObject);
            advance_cutscene_step(m);
        }
    }
}
...
static void jumbo_star_cutscene_taking_off(struct MarioState *m) {
    ...
    if (m->actionState == ACT_STATE_JUMBO_STAR_CUTSCENE_TAKING_OFF_DANCE) {
        ...
        switch (animFrame) {
            ...
            case 28:
                play_sound(SOUND_CHARACTER_HOOHOO, marioObj->header.gfx.cameraToObject);
                break;

            case 60:
                play_sound(SOUND_CHARACTER_YAHOO, marioObj->header.gfx.cameraToObject);
                break;
        }
        m->particleFlags |= PARTICLE_SPARKLES;
