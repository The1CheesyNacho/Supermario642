...
static void common_water_knockback_step(struct MarioState *m, s32 animation, u32 endAction, s32 actionArg) {
    ...
    if (is_anim_at_end(m)) {
        ...
        set_mario_action(m, gMarioStates[0].health >= 0x100 ? endAction : ACT_WATER_DEATH, 0);
    }
}
...
static s32 act_water_shocked(struct MarioState *m) {
    ...
    if (m->actionTimer >= 6) {
        m->invincTimer = 30;
        set_mario_action(m, gMarioStates[0].health < 0x100 ? ACT_WATER_DEATH : ACT_WATER_IDLE, 0);
    }
...
static s32 act_drowning(struct MarioState *m) {
    ...
    play_sound_if_no_flag(m, SOUND_CHARACTER_DROWNING, MARIO_ACTION_SOUND_PLAYED);
    stationary_slow_down(m);
    perform_water_step(m);
...
static s32 act_water_plunge(struct MarioState *m) {
    ...
    if (m->actionState == ACT_STATE_WATER_PLUNGE_FALL) {
        ...
        if (m->peakHeight - m->pos[1] > FALL_DAMAGE_HEIGHT_SMALL) {
            play_sound(SOUND_CHARACTER_HAHA_WATER, m->marioObj->header.gfx.cameraToObject);
        }
...
static s32 check_common_submerged_cancels(struct MarioState *m) {
    ...
    if (gMarioStates[0].health < 0x100 && !(m->action & (ACT_FLAG_INTANGIBLE | ACT_FLAG_INVULNERABLE))) {
