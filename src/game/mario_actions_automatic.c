...
s32 act_holding_pole(struct MarioState *m) {
    ...
    if ((m->input & INPUT_Z_PRESSED) || gMarioStates[0].health < 0x100) {
...
s32 act_ledge_climb_slow(struct MarioState *m) {
    ...
    if (m->actionTimer == 10) {
        play_sound_if_no_flag(m, SOUND_CHARACTER_EEUH, MARIO_MARIO_SOUND_PLAYED);
    }
...
s32 act_ledge_climb_fast(struct MarioState *m) {
    ...
    play_sound_if_no_flag(m, SOUND_CHARACTER_UH_LEDGE_CLIMB_FAST, MARIO_MARIO_SOUND_PLAYED);
