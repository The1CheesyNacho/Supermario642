...
//	o->oParentRelativePosY = 0.0f;
	...
	switch (o->parentObj->oMotosUnk88 ){
		...
		case	1:
			...
			break;
		case	2:
                gMarioObject->oInteractStatus |= (INT_STATUS_MARIO_DROPPED_BY_OBJ + INT_STATUS_MARIO_THROWN_BY_OBJ);
...
void motos_carry_start(void)
{
	...
	cur_obj_init_animation_with_sound(3);
			cur_obj_play_sound_2(SOUND_ACTION_UNK3C);
	}
...
void motos_carry_run(void)
{
	...
	cur_obj_rotate_yaw_toward(o->oAngleToMario+o->oMotosUnk9F, o->oMontyMoleHeightRelativeToFloor);
		if (cur_obj_check_anim_frame(2)| cur_obj_check_anim_frame(11))  cur_obj_play_sound_2(SOUND_OBJ_POUNDING1);
...
void motos_pitch(void)
{
	...
	if ( cur_obj_check_anim_frame(7) ) cur_obj_play_sound_2(SOUND_ACTION_THROW);
...
void motos_fly(void)
{
	cur_obj_init_animation_and_extend_if_at_end(5);
		if (((gCurrLevelNum == NULL) & (o->oPosY < 1000.0f)) || ((o->oFloor->type == SURFACE_BURNING) && ( o->oMoveFlags & OBJ_MOVE_LANDED))) {
...
void motos_pitch2(void) //Throwing Fire action for Fire Motos
{
	...
	if (o->oTimer == 0) cur_obj_play_sound_2(SOUND_ACTION_UNK3C);

	if ( cur_obj_check_anim_frame(9) ) cur_obj_play_sound_2(SOUND_ACTION_THROW);
...
void s_motos(void)
{
	...
	switch (o->oHeldState) {
        case HELD_FREE:
            ...
            break;
        case HELD_HELD:
            cur_obj_unrender_set_action_and_anim(8, 1);
            break;
        case HELD_THROWN:
        case HELD_DROPPED:
